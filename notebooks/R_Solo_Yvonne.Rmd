...
library(tidyverse)
```
1. Read in `districts.csv` into a tibble named `districts`. How many rows and columns does it contain? 146 Rows and 27 Columns
...
2. Notice that the first row corresponds to the whole State of Tennessee. Remove this row and save the result back to `districts`.
```{r}
districts.1 <- districts[2:146, ]
```

3. How many districts have a proficiency rate of at least 80% for both alg_1 and eng_1? 13
```{r}
districts.1 %>% 
...
```{r}
#setDT(districts.1)[,?.SD[which.min(grad)], by = system_name]
```
```{r}
districts.1 %>% 
  slice_min(grad)
